...
/*    ////////////time1
    ...
    for(ishot=1;ishot<=nshot;ishot++)//////////////////////// nested for start
    {
        printf("shot=%d\n",ishot);
	flog = fopen(logfile,"a");
        fprintf(flog,"shot=%d\n",ishot);
	fclose(flog);
        ...
        //#pragma omp parallel for/////////////////////// valied
        for(int i=0;i<nz;i++)
            for(int j=0;j<ny;j++)
                for(int k=0;k<nx;k++)
                {
...
//    #pragma omp parallel for///////////////////////// (1)->66/(1,2)->100
        for(l=1;l<=lt;l++)
        {
            xmax=l*dt*velmax;
            ...
            nfront = nfront-1;
            nleft = nleft-1;
            #pragma omp parallel for shared(wave,c,u,v,w,up,up1,up2,vp,vp1,vp2,wp,wp1,wp2,us,us1,us2,vs,vs1,vs2,ws,ws1,ws2,vss,vpp,dr1,dr2,dtz,dtx,ncx_shot,ncy_shot,ncz_shot,ntop,nleft,nfront,nbottom,nback,nright)
            for(int k=ntop;k<nbottom;k++)//////////////////////loop1
                for(int j=nfront;j<nback;j++)
                    for(int i=nleft;i<nright;i++)
                    {
                        ...
                        float vvp2=vpp[k*ny*nx+j*nx+i]*vpp[k*ny*nx+j*nx+i];

                        float vvs2=vss[k*ny*nx+j*nx+i]*vss[k*ny*nx+j*nx+i];
            ...
            #pragma omp parallel for shared(u,v,w,up,up1,up2,vp,vp1,vp2,wp,wp1,wp2,us,us1,us2,vs,vs1,vs2,ws,ws1,ws2)
            for(int k=ntop;k<nbottom;k++)/////////////////// loop 2
                for(int j=nfront;j<nback;j++)
                    for(int i=nleft;i<nright;i++)
                    {
